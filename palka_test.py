...
# ---------------- ножки (мотор торчит вниз на 22+3.35 разъём) ----------------
leg = Pos(0, 0, 13) * Cylinder(5, 26)
leg += Pos(0, 0, 27.5) * Cylinder(2.35, 3.0)   # шип в отверстие плиты
# ---------------- фальш-мотор (игра без мотора): заглушка в колодец ------
plug = Pos(0, 0, 1) * Cylinder(20.5, 2)        # фланец сверху
plug += Pos(0, 0, -1.5) * Cylinder(18.5, 3.1)  # пенёк в колодец
plug -= Pos(0, 0, 0.5) * Cylinder(PIN, 5.5)    # ось кривошипа (штифт)

parts = [("ptest_plate_v1", plate), ("ptest_triangle_v1", tri),
         ("ptest_leg_v1", leg), ("ptest_plug_v1", plug),
