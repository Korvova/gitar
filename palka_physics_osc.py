...
POS = {
    ...
    "ptest_bridge_v1#2": ((186, -16, 10.0), 0),
    "ptest_washer_v1": ((95, 0, 6.9), 0),
    "ptest_bridge_low_v1": ((65, -7, 8.4), 0),
...
c_box(ca, 7.5, 9.65, -18, 12, 4.5, 8)         # палуба: 3 бокса вокруг ВИЛКИ
c_box(ca, -9.65, 7.5, -18, -11, 4.5, 8)       # (открыта на запад)
c_box(ca, -9.65, 7.5, -5, 12, 4.5, 8)
...
# кулиса: торец вилки углублён (7.5) и лимиты с запасом — при повороте
# осей узла (±28 с треугольником) штырь не дожимается до торца
cs = slop("j_slot", 30, 0, 11, OBJ["ptest_triangle_v2"], OBJ["ptest_cart_v2"], -0.5, 3.2)
cs.limit_lin_x_lower = -7.0                   # вилка: штырь гуляет по дуге на запад
cs.limit_lin_x_upper = 2.0
...
bpy.ops.wm.save_as_mainfile(filepath=r"C:\App\gitar\2-0\Стенд_качание.blend")
print("PHYS OK")
