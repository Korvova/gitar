...
Спицы-ленты 8×1.6 всех этажей бегут друг над другом в жёлобе x 11..21.
...
def slot_cut(yc, z0, z1):
    """Дуговая прорезь под штырь плеча: X ±23.5, Y yc-3.5..yc+7.5."""
    return BB(-23.5, 23.5, yc - 3.5, yc + 7.5, z0, z1)
...
def walls(z0, z1, south_open=True):
    ...
    if south_open:
        w += BB(-24, 10, L - 2, L, z0, z1) + BB(22, 24, L - 2, L, z0, z1)
    else:
...
for k in range(4):
    ...
    a -= Pos(0, 0, 0.8) * Cylinder(3.0, 1.8)             # дырка оси Ø6
...
cart -= BB(-3, 3, -7.1, 3, -0.1, CART_H + 0.1)           # вилка: паз 6, открыт на юг
carts = cart

# ---------------- спицы-ленты: головы с кольцом и ступенькой ---------------
spicas = []
for k in range(4):
    ln = 170 - AXES_Y[k]                   # до юга секции (+хвост под нахлёст)
    ...
    s -= Pos(0, 4, 2.6) * Cylinder(3.0, 1.8)             # на стад угла-1
    s += BB(-1.5, 1.5, ln, ln + 3, 0, 1.6)               # ПАЗЛ-гриб: стык сегментов
    s += Pos(0, ln + 4.2, 0.8) * Cylinder(2.6, 1.6)      # лент (плоский, без болтов)
    spicas.append(s)
...
for k in range(4):
    for phi, tag in ((0, "n"), (22.6, "p"), (-22.6, "m")):
        ...
        pairs_clear += [(nm, f"p{k}" if k else "p0", 0.1)]   # vs своя плита (кроме стада)
