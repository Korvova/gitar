# -*- coding: utf-8 -*-
"""ГИТАРА: СЕКЦИЯ-1 v2 (26.09.2026) — тележки на шаге V позиции 26/25/24 мм.
    ...
    for y in range(5, 156, 10):
        if all(not (lo <= y <= hi) for lo, hi in bad):
            if not out or y - out[-1] >= 30:
                out.append(y)
    return out
TIE = [(23.5, 6), (-23.5, 6), (23.5, 120), (-23.5, 120), (23.5, 150), (-23.5, 150)]   # v0: средняя на y=80
...
# паз под штырь: по X плотно (FIT_D+0.1), по Y закрыт и длиннее — штырь уходит на юг на 4 мм
# в крайних положениях плеча и тележку по Y не толкает (её держит русло)
cart -= BB(-(FIT_D + 0.1) / 2, (FIT_D + 0.1) / 2, -2.9, 6.9, -0.1, CART_H + 0.1)
carts = cart
...
kupon_stud += BB(-3, 3, -40, 0, 0, 2)                        # рычаг-ручка 40 мм: крутить и чувствовать посадку
...
for name, part in parts:
    ...
    export_stl(p, rf"{OUT}\{name}_v2.stl")
...
SRC = OUT
asm = Assembly()
asm.add("p0", SRC + r"\gs1_p0_base_v2.stl")
for k in (1, 2, 3):
    asm.add(f"p{k}", SRC + rf"\gs1_p{k}_mid_v2.stl", loc=(0, 0, Z_FLOOR[k] - FLOOR))
asm.add("deck", SRC + r"\gs1_p4_deck_v2.stl", loc=(0, 0, Z_DECK))
pairs_touch = [("p0", "p1"), ("p1", "p2"), ("p2", "p3"), ("p3", "deck")]
pairs_clear = []
for k in range(4):
    for phi, tag in ((0, "n"), (22.6, "p"), (-22.6, "m")):
        nm = f"arm{k}{tag}"
        asm.add(nm, SRC + rf"\gs1_arm{k}_v2.stl",
                loc=(0, AXES_Y[k], Z_FLOOR[k] + 0.2), rz=phi)
        up = f"p{k + 1}" if k < 3 else "deck"
        pairs_clear += [(nm, up, 0.15)]                  # плечо/штырь vs плита выше
        pairs_clear += [(nm, f"p{k}" if k else "p0", 0.01)]  # vs своя плита: стад в дырке сидит плотно (FIT_D 5.1: радиальный зазор 0.05, фасеты STL съедают его почти в ноль)
    pairs_clear += [(f"sp{k}", f"p{k}" if k else "p0", 0.1)]     # спица vs своя плита: стенки, колонны, южный проход
    asm.add(f"sp{k}", SRC + rf"\gs1_spica{k}_v2.stl",
            loc=(LANE_X, AXES_Y[k] - 4, Z_FLOOR[k] + 0.2))
    up = f"p{k + 1}" if k < 3 else "deck"
    pairs_clear += [(f"sp{k}", up, 0.15)]
    asm.add(f"cart{k}", SRC + r"\gs1_cart_v2.stl", loc=(0, CARTS_Y[k], Z_DECK + DECK_T))
    pairs_touch += [(f"cart{k}", "deck")]
    for phi, tag in ((22.6, "p"), (-22.6, "m")):         # тележка едет за штырём: штырь в пазу, тележка в русле
        cn = f"cart{k}{tag}"
        asm.add(cn, SRC + r"\gs1_cart_v2.stl",
                loc=(52 * _m.sin(_m.radians(phi)), CARTS_Y[k], Z_DECK + DECK_T))
        pairs_touch += [(cn, "deck")]
        pairs_clear += [(f"arm{k}{tag}", cn, 0.0)]
