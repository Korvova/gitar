# -*- coding: utf-8 -*-
"""Сцена Blender «Гитара_дека_v2.blend»: хребет деки v2 с моторами Ø36, кривошипы,
хвосты лент, гребёнки, секция-3 с лентами. Анимация: кривошипы качаются ±38°,
ленты ходят вместе с пальцами (этажи вразнобой). Пробел — проиграть.
...
OUT = os.path.join(ROOT, "2-0", "Гитара_дека_v2.blend")

Z_FLOOR = [3, 8.4, 13.8, 19.2]
MY = [515, 565, 615, 678]
COMB_Y = (540, 590, 634)
LANE, R_PIN, SWING = 10, 7.6, 38.0
FRAMES = 96
...
imp("gdk1_base_v2", "Д1_хребет", (0, 480, 0), c_deka, C_BASE)
imp("gdk2_base_v2", "Д2_хребет", (0, 640, 0), c_deka, C_BASE)
for i, yg in enumerate(COMB_Y):
    imp("gdk_comb", f"гребёнка_{i}", (0, yg, 0), c_deka, C_COMB)
for k, my in enumerate(MY):
    imp(f"gdk_motor{k}_model", f"мотор_{k}", (0, my, 0), c_mot, C_MOT)
...
for k, (my, zf) in enumerate(zip(MY, Z_FLOOR)):
    cranks.append(imp(f"gdk_crank{k}_v2", f"кривошип_{k}", (0, my, 0), c_mech, C_CRANK))
    tails.append(imp(f"gdk_tail{k}_v2", f"хвост_{k}", (LANE, 480, zf + 0.05), c_mech, C_TAIL))
    exts.append(imp("gdk_ext", f"лента_секция3_{k}", (LANE, 320, zf + 0.05), c_sec, C_EXT))

# анимация качания: этажи 0 и 2 в фазе, 1 и 3 в противофазе
for f in range(1, FRAMES + 1, 2):
    ph = 2 * math.pi * (f - 1) / (FRAMES - 1)
    for k in range(4):
        th = SWING * math.sin(ph) * (1 if k % 2 == 0 else -1)
        dy = R_PIN * math.sin(math.radians(th))
