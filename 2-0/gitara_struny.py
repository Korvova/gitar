...
def make_garm3(t):
    """пружина моноблока (юзер): выходит ИЗ ТОРЦА струны по её оси —
    без лапки и нахлёста; зигзаг, прямой хвост, один изгиб к столбику"""
    h = t / 2
    g = BB(-0.6, 0.6, -1, 1, 0, 6)                    # корешок в торец струны
    yy = 1.0
    for i in range(3):
        dx = 4 if i % 2 == 0 else -4
        g += BB(min(0, dx) - h, max(0, dx) + h, yy, yy + 2 * h, 0, 6)
        ...
        g += BB(min(0, dx) - h, max(0, dx) + h, yy + 3.8 - 2 * h, yy + 3.8, 0, 6)
        yy += 3.8
    g += BB(-0.6, 0.6, yy, 29.4, 0, 6)                # прямой хвост по оси
    g += BB(-0.6, 6.2, 29.4, 30.6, 0, 6)              # изгиб к столбику сбоку
    g -= Pos(3.4, 30.0, 3) * Rot(90, 0, 0) * Cylinder(0.8, 3.2)
    return g
...
# чистая струна для палки: без вырезов под крышечки хвостов (хвостов нет)
# и без дырок винтов гармошек (пружина цельная); только шплинт обоймы
struna_clean = BB(-1, 1, 0, 100, 0, 10)
struna_clean -= Pos(0, 50, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 3)


def make_palka(t, marks):
    g = make_garm3(t)
    pk = struna_clean
    pk += Pos(0, 99, 0) * g                            # юг: из торца струны
    pk += Pos(0, 1, 0) * Rot(0, 0, 180) * g            # север (зеркало)
    for k in range(marks):                             # метки жёсткости
        xk = 3.5 + (k - (marks - 1) / 2) * 2.0
        pk -= BB(xk - 0.4, xk + 0.4, 128.5, 129.5, 5.4, 6.05)
    return pk
...
# ========== МОДУЛЬ-КАРТРИДЖ струны на 49E (переосмысление юзера) ========
# независимая струна: планка 12x175 с ИНТЕГРИРОВАННЫМИ столбиками и
# гнездом холла 49E. Магнит - в короткой обойме C у ВСЕХ струн (49E не
# боится соседних магнитов). Датчик вдвигается в туннель сбоку (с юга)
# ПЛОСКОЙ СТОРОНОЙ С НАДПИСЬЮ ВВЕРХ, ножки вниз через прорезь, провода
# под планкой: VCC/GND на шины, OUT -> свой ADC-пин ESP32.
# Струна ниже, чем в раме v3: низ на z9 (зазор магнит-датчик ~2.5)
mod = BB(-6, 6, -87.5, 87.5, 0, 3)                     # база 12 x 175
for my_ in (82, -82):
    mod -= Pos(0, my_, 1.5) * Cylinder(1.7, 3.2)       # крепёж М3 к чему угодно
for sgn in (1, -1):                                    # столбики СБОКУ от оси
    mod += BB(min(1 * sgn, 6 * sgn), max(1 * sgn, 6 * sgn),
              sgn * 75.4 - 3, sgn * 75.4 + 3, 3, 16)
    mod -= Pos(3.4 * sgn, sgn * 75.4, 12) *         Rot(90, 0, 0) * Cylinder(1.1, 6.4)             # винт плеча палки
mod += BB(-4.6, 2.4, -4, 4, 3, 5.2)                    # прилив гнезда 49E
...
vstavka = BB(-3.8, 1.6, -9.4, -2.8, -1, 0)             # буртик
vstavka += BB(-2.7, 0.5, -8.4, -3.8, -1, 3)            # тело 3.2x4.6: уже
vstavka -= BB(-2.1, -0.1, -7.8, -4.4, -1.1, 3.1)       # прорези 4.4x5.2 —
                                                       # вокруг остаётся щель
                                                       # проводам (юзер)
