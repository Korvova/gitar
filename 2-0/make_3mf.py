...
plates = [
    ...
    ("71_hall_posts", p020_j,              # стойки 49E в деку v3, этажи 0-3 (клей на дно хребта)
     prep("gdk_hall_post0_v1") + prep("gdk_hall_post1_v1") + prep("gdk_hall_post2_v1") + prep("gdk_hall_post3_v1")),
    ...
    # ---- ДЕКА v3 (13.09.2026): полный оборот кривошипа (кулиса) под моторы
    # Ø36. Кривошип: пальцем на стол (FLIP), под диском поддержка от стола.
    # Ленты-продолжения со стола 36 не перепечатывать
    ("66_kupon_shesternya", p020_j,        # купон: 4 круглые трубки (риски 1-4)
     prep("gdk_kupon_shesternya", ID)),    # + 2 звёздочки z10 (риски 5-6)
    ("67_deka1_v3", p020_j,                # хребет Д1, кривошипы 0-2, проставка
    ...
    ("64_vstavki", p020_j,                 # вставки-фиксаторы проводов
     prep("gst_vstavka", ID, copies=8)),   # в прорези модулей (6 + запас)
    ...
    ("62_moduli5_vse", p020_j,             # ВСЕ 5 оставшихся картриджей разом:
     ...
     prep("gst_palka06", ID, copies=2) +   # струны 2,5
     prep("gst_palka07", ID) +             # струна 6
     prep("gst_skoba", LAY, copies=5)),    # обоймы магнитов
    # ниже — те же картриджи по одному на стол (если печатать порознь);
    # жёсткости чередуются: струны 1,4 = 0.8 (3 риски), 2,5 = 0.6 (1), 6 = 0.7 (2)
    ...
    ("55_palki6", p020_j,                  # ПАЛКИ-моноблоки на все 6 струн:
     ...
     prep("gst_palka07", ID, copies=2)),   # 2 риски - струны 3,6
