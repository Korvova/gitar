...
# магнит AS5600 по ЗАМЕРУ юзера: Ø4.1 x 2.1, ПЛАШМЯ (диаметральный!)
struna2 = BB(-1, 1, 0, 100, 0, 10)                     # симметричная пластина
for hy in (12, 88):
    struna2 -= Pos(0, hy, 3) * Rot(0, 90, 0) * Cylinder(1.1, 3)
# бобышка магнита (на восток — печать лёжа чистая) с ямкой СНИЗУ и
# канавками под зубья П-обоймы (конструкция юзера)
struna2 += BB(-1, 4, 46, 54, 0, 6)
struna2 -= Pos(1.5, 50, 1.05) * Cylinder(2.13, 2.3)    # ямка магнита снизу
struna2 -= BB(-1, -0.6, 45.9, 54.1, 3, 3.8)            # канавки защёлки
struna2 -= BB(3.6, 4, 45.9, 54.1, 3, 3.8)

# П-ОБОЙМА магнита: обхватывает бобышку снизу, зубья в канавки, дно 1 мм
# запирает магнит; лок: центр бобышки (1.5, 50) -> (0, 0)
sk = BB(-3.8, 3.8, -4, 4, -1, 0)                       # дно
sk += BB(-3.8, -2.6, -4, 4, 0, 3.9)                    # стенки
sk += BB(2.6, 3.8, -4, 4, 0, 3.9)
sk += BB(-2.7, -2.3, -4, 4, 3, 3.8)                    # зубья внутрь
sk += BB(2.3, 2.7, -4, 4, 3, 3.8)
skoba = sk
...
tp2 += BB(-11.3, 14.3, -12.8, 12.8, 3, 7)              # подиум платы AS5600
tp2 -= BB(-9.9, 12.9, -11.4, 11.4, 5.5, 7.1)   # карман 22.8 (центр чипа под
tp2 -= BB(-20.1, -8, -3, 3, 5.4, 7.1)          # магнитом x=+1.5); паз проводов
