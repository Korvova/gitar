...
def make_garm(t, mono=False):
    """t — толщина стенки зигзага (жёсткость); mono — хвост для
    палки-моноблока: прямое продолжение + один изгиб к столбику СБОКУ
    (идея юзера), вместо Г-обхода столбика на оси"""
    h = t / 2
    ...
    for i in range(3):
        ...
        yy += 3.8
    if mono:
        g += BB(-0.6, 0.6, yy, 33.7, 0, 6)            # прямое продолжение
        g += BB(-0.6, 6.2, 32.5, 33.7, 0, 6)          # изгиб к столбику сбоку
        g -= Pos(3.4, 33.1, 3) * Rot(90, 0, 0) * Cylinder(0.8, 3.2)
    else:
...
def make_palka(t, marks):
    ...
    for k in range(marks):                             # метки жёсткости
        ...
        pk -= BB(xk - 0.4, xk + 0.4, 128.6, 129.6, 5.4, 6.05)
    return pk
...
for sgn in (1, -1):                                    # столбики СБОКУ от оси
    mod += BB(min(1 * sgn, 6 * sgn), max(1 * sgn, 6 * sgn),
              sgn * 75.5 - 3, sgn * 75.5 + 3, 3, 16)
    mod -= Pos(3.4 * sgn, sgn * 75.5, 12) *         Rot(90, 0, 0) * Cylinder(1.1, 6.4)             # винт плеча палки
