...
struna2 = BB(-1, 1, 0, 100, 0, 10)                     # симметричная пластина
for hy in (12, 88):
    struna2 -= Pos(0, hy, 3) * Rot(0, 90, 0) * Cylinder(1.1, 3)
struna2 -= BB(-1.1, 1.1, 47.85, 52.15, -0.1, 3.2)      # щель под магнит
struna2 -= Pos(0, 50, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 3)   # шплинт

# П-коробочка (перевёрнутая П): дно с окном (магнит ближе к чипу), две
# стенки обхватывают тело, шплинт сквозь; печать ЛЁЖА на боку профилем
sk = BB(-2.3, 2.3, -4, 4, -2, -1)                      # дно
sk -= Pos(0, 0, -1.5) * Cylinder(1.6, 1.2)             # окно под магнитом
sk += BB(-2.3, -1.15, -4, 4, -1, 6)                    # стенки (проём 2.3)
sk += BB(1.15, 2.3, -4, 4, -1, 6)
sk -= Pos(0, 0, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 6)   # шплинт сквозь
skoba = sk

tp2 = BB(-20, 20, -85, 85, 0, 3)                       # плита v2
for sy, sxs in ((-67.5, -2.2), (67.5, 2.2)):           # каналы 2 столбиков
    for dy in (-6, 6):
        tp2 -= Pos(sxs, sy + dy, 1.6) * Cylinder(0.9, 2.9)
tp2 += BB(-12.8, 12.8, -12.8, 12.8, 3, 6)              # подиум платы AS5600
tp2 -= BB(-11.4, 11.4, -11.4, 11.4, 4.4, 6.1)          # карман 22.8, центр x0
tp2 -= BB(-20.1, -9, -3, 3, 4.3, 6.1)                  # паз проводов на запад
