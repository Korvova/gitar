...
# П-коробочка: магнит лежит ПЛАШМЯ в ямке на дне обоймы (юзер) — никакой
# ориентации полюсов не нужно; обойма шплинтом на струне, ездит с ней.
# лок: z0 = НИЗ струны
sk = BB(-3, 3, -4, 4, -2.6, 0)                         # дно с ямкой магнита
sk -= Pos(0, 0, -1.075) * Cylinder(2.13, 2.25)         # ямка Ø4.25 x 2.2
sk += BB(-3, -1.15, -4, 4, 0, 6)                       # стенки (проём 2.3)
sk += BB(1.15, 3, -4, 4, 0, 6)
sk -= Pos(0, 0, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 7)   # шплинт сквозь
skoba = sk
