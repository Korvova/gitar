...
# ---- ямка магнита; для открытой (на хвосте) — КЛИН-защёлки (юзер) ------
# профиль зубца: наклонный заход сверху (~35град), горизонтальный стоп снизу.
# РАСЧЁТ: ямка 2.25, магнит 2.1 -> верх магнита на -0.1; стоп-полка на +0.05
# -> зазор 0.15, магнит уходит под зубцы ПОЛНОСТЬЮ. Навес внутрь 0.275.
_TOOTH = [(1.775, 0.05), (2.2, 0.05), (2.2, -0.25), (2.65, -0.25),
          (2.65, 0.95), (1.775, 0.35)]


def magnet_pocket(part, cx, cy, teeth=False):
    part -= Pos(cx, cy, -1.075) * Cylinder(2.125, 2.25)    # ямка Ø4.25 (туже)
    if teeth:
...
sk = magnet_pocket(sk, -1.1, 0)          # без зубцов: магнит накрыт струной
...
sk6 += BB(-3, 1, 4, TAIL_Y - 4, -2.6, 0)              # хвост 4x2.6
sk6 += BB(-4.6, 2.4, TAIL_Y - 4, TAIL_Y + 4, -2.6, 0)  # площадка магнита
sk6 = magnet_pocket(sk6, -1.1, TAIL_Y, teeth=True)     # клин-защёлки
skoba6 = sk6
...
for i, sx in enumerate(SX):
    ...
    PCB.append((mx, my))
    rama += BB(mx - 12.8, mx + 12.8, my - 12.8, my + 12.8, 3, 7.8)
    rama -= BB(mx - 11.4, mx + 11.4, my - 11.4, my + 11.4, 6.1, 7.9)
    rama -= BB(mx - 11.4, mx - 3.5, my - 9, my + 9, -0.1, 6.2)   # пины запад
    rama -= BB(mx + 3.5, mx + 11.4, my - 9, my + 9, -0.1, 6.2)   # пины восток
