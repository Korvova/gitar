...
# КРЫШКА ПЛАТЫ (юзер): плата не торчит голой — пластинка 26x26x1 на тех же
# двух М3, просто дырки. Карман углублён до 3 мм (высота платы с чипом —
# замер юзера), плата утопает целиком. Два варианта: с окном над чипом и
# ГЛУХАЯ (магнит читает сквозь 1 мм пластика — пластик полю не мешает).
# лок: z0 = верх подиума (8.5 в раме); хвост обоймы идёт на 10.9
def make_kr_pcb(okno):
    kr = BB(-13, 13, -13, 13, 0, 1.0)
    if okno:
...
for i, sx in enumerate(SX):
    ...
    rama -= BB(mx - 11.8, mx + 11.8, my - 11.8, my + 11.8, 5.5, 8.6)  # карман
                     # 23.6 (юзер шлифовал 22.8); глубина 3 — плата утопает
    ...
    # хрупкие -> ДВА штырька Ø3.1 (направляйки) + ДВА винта М3 по диагонали
    for px in (-8, 8):
        for py in (-8, 8):
            if px * py < 0:                            # диагональ: каналы М3
                rama -= Pos(mx + px, my + py, 3) * Cylinder(1.35, 6.6)
            else:                                      # диагональ: штырьки
                rama += Pos(mx + px, my + py, 6.5) * Cylinder(1.55, 2.0)
...
cell -= BB(-11.8, 11.8, -11.8, 11.8, 5.5, 8.6)         # карман 23.6 гл.3
...
for px in (-8, 8):
    for py in (-8, 8):
        if px * py < 0:                                # диагональ: каналы М3
            ...
            cell += Pos(px, py, 6.5) * Cylinder(1.55, 2.0)
