...
# юг: боковой ПРИЛИВ (вертикальная грань) — лапка гармошки сбоку + винт М2
struna += BB(STR_T / 2, STR_T / 2 + 2.5, STR_L - 16, STR_L - 8, 0, 6)
struna -= (Pos(1.2, STR_L - 12, 3) * Rot(0, 90, 0) * Cylinder(0.9, 5.2))
...
for i in range(3):                                     # колена зигзага
    ...
    yy += 3.8
g += BB(-0.6, 0.6, yy, 24, 0, 6)                      # полоса
g += BB(-3, 3, 24, 25.5, 0, 6)                        # южная лапка (поперечная)
g -= Pos(0, 24.75, 3) * Rot(90, 0, 0) * Cylinder(1.1, 2)
garm = g
...
stolb += BB(-4, 4, -4, 4, 3, 17.5)                    # тело
for dy in (-6, 6):
    stolb -= Pos(0, dy, 1.5) * Cylinder(1.1, 3.2)
stolb -= (Pos(0, -0.75, 13.5) * Rot(90, 0, 0) *
          Cylinder(0.9, 6.7))                          # канал лапки (с севера)
