...
# ---- ямка магнита с ЗУБЦАМИ-защёлкой (юзер: чтобы не вылетел) ----------
# магнит продавливается вниз мимо трёх навесов 0.3 и запирается
import math as _m


def magnet_pocket(part, cx, cy):
    part -= Pos(cx, cy, -1.075) * Cylinder(2.125, 2.25)    # ямка Ø4.25 (туже)
    for ang in (90, 210, 330):                             # 3 зубца сверху
        a = _m.radians(ang)
        part += (Pos(cx + 2.0 * _m.cos(a), cy + 2.0 * _m.sin(a), -0.15) *
                 Rot(0, 0, ang) * Box(0.5, 1.4, 0.3))
    return part
...
sk6 = magnet_pocket(sk6, -1.1, TAIL_Y)                 # ямка с зубцами
skoba6 = sk6
