...
struna = BB(-STR_T / 2, STR_T / 2, -3.5, STR_L - 8, 0, STR_H)  # тело; упор в
                                                # ротор — сама нижняя грань (юзер)
# лопатка НАСКВОЗЬ ротора и корпуса, ДЛИННОЙ стороной вдоль струны;
# ниже корпуса — дырка Ø1.6: болтик М2 поперёк = замок (юзер)
struna += BB(-STR_T / 2, -STR_T / 2 + BLADE_T,     # лопатка У КРАЯ тела (юзер):
             -BLADE_W / 2, BLADE_W / 2,               # при печати лёжа лежит на
             -(ROT_H + POT_H + 3), 0.1)               # столе, а не висит
...
# ==== ТЕСТ v2 «как настоящая струна» (юзер): БЕЗ потенциометра =========
# струна симметричная между ДВУМЯ гармошками на двух столбиках; магнит
# AS5600 клеится на дно по центру, плата-энкодер лежит на плите под ним
# (зазор магнит-чип ~2.4). Гармошка та же х2 (вторая развёрнута на 180°),
# столбик тот же х2 (допечатать один).
# магнит AS5600 по ЗАМЕРУ юзера: Ø4.1 x 2.1. Конструкция юзера (эскиз):
# струна ПЛОСКАЯ, в нижней кромке ЩЕЛЬ под магнит (боком, полюса вдоль
# струны — сориентировать при сборке вторым магнитом), снизу перевёрнутая
# П-коробочка, шплинт-филамент сквозь П и тело. Всё печатается без поддержек.
struna2 = BB(-1, 1, 0, 100, 0, 10)         # ПЛОСКАЯ пластина, даже без щели
for hy in (4, 96):                     # дырки у самых концов — зигзаг гармошки
    struna2 -= Pos(0, hy, 3) * Rot(0, 90, 0) * Cylinder(1.1, 3)   # мимо тела
struna2 -= Pos(0, 50, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 3)   # шплинт обоймы

# Обойма магнита = перевёрнутая Г (юзер): дно с ямкой + ОДНА стенка —
# ямка открыта сбоку, магнит кладётся плашмя свободно; вторая щека —
# отдельная плоская пластинка; шплинт сквозь всё: пластина-тело-стенка.
# лок: z0 = НИЗ струны
sk = BB(-3.6, 3, -4, 4, -2.6, 0)                       # дно с ямкой магнита
sk -= Pos(-1.1, 0, -1.075) * Cylinder(2.2, 2.25)   # ямка Ø4.4, СДВИНУТА от
                                                   # стенки — магнит кладётся
                                                   # сверху свободно (юзер)
...
# ====== Г-обойма С ХВОСТОМ (6-струнная компоновка, идея юзера) ==========
# магнит выносится вбок на конце жёсткого хвоста — платы AS5600 встают
# в 2 ряда через одну струну; хвост у всех одной длины (ямка на y=26)
sk6 = BB(-3.6, 3, -4, 4, -2.6, 0)                      # дно обоймы (без ямки)
sk6 += BB(1.15, 3, -4, 4, 0, 6)                        # стенка Г
sk6 -= Pos(0, 0, 4.4) * Rot(0, 90, 0) * Cylinder(0.85, 8)   # шплинт
sk6 += BB(-3, 1, 4, 22, -2.6, 0)                       # хвост 4x2.6
sk6 += BB(-4.6, 2.4, 22, 30, -2.6, 0)                  # площадка магнита
sk6 -= Pos(-1.1, 26, -1.05) * Cylinder(2.2, 2.25)      # ямка Ø4.4 (вверх)
skoba6 = sk6
# подставка под плату AS5600 для теста хвоста на текущем стенде (плата
# уезжает с подиума на 26 мм — подставка держит тот же зазор до магнита)
podstavka = BB(-12.4, 12.4, -12.4, 12.4, 0, 4.8)

tp2 = BB(-20, 20, -95, 95, 0, 3)                       # плита v2
for sy, sxs in ((-75.5, -2.2), (75.5, 2.2)):   # каналы 2 столбиков (плечо
    for dy in (-6, 6):                         # гармошки — СНАРУЖИ тела)
        tp2 -= Pos(sxs, sy + dy, 1.6) * Cylinder(0.9, 2.9)
# подиум платы: сдвинут на x=-1.1 вслед за магнитом (чип под ним)
tp2 += BB(-13.9, 11.7, -12.8, 12.8, 3, 7.8)
tp2 -= BB(-12.5, 10.3, -11.4, 11.4, 6.1, 7.9)          # карман: чип на ~8.6
tp2 -= BB(-20.1, -10.1, -3, 3, 6.0, 7.9)               # паз проводов
tp2 -= BB(-12.5, -4.6, -9, 9, -0.1, 6.2)       # СКВОЗНЫЕ прорези под штырьки,
tp2 -= BB(2.4, 10.3, -9, 9, -0.1, 6.2)         # широкие до перемычки (юзер)
